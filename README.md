...
The PC Bridge directory contains a desktop app built on Electron and Node.js that connects directly to MSFS 2024 and acts as a telemetry and command relay:
...
The user-facing dashboard hosted on GitHub Pages is optimized for high resolution mobile phone or tablet displays. It is a Progressive Web App (PWA) featuring viewport constraints to prevent browser scrolling and gesture bounciness:
...
##PWA Offline Support & Install: Implements service worker caching (sw.js) and a custom "Install App" modal with explicit, platform-specific steps for adding the app to the iOS Home Screen via Safari or Chrome's app launcher.
